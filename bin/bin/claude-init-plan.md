# Plan: Claude Init Script for Local Projects

## Overview

Create a bash script `claude-init` that initializes Claude Code configuration for local projects.

## Files to Create

| File | Phase |
|------|-------|
| `~/dotfiles/bin/bin/templates/CLAUDE.md.template` | 1 |
| `~/dotfiles/bin/bin/claude-init` | 2-3 |
...
```bash
mkdir -p ~/dotfiles/bin/bin/templates
```

### 1.2 Create `templates/CLAUDE.md.template`
...
## Phase 2: Core Script with Non-Interactive Mode

### 2.1 Create `claude-init` script
...
main() {
    ...
    log_warning "Interactive mode not yet implemented. Use --all flag."
    exit 1
}
...
```bash
chmod +x ~/dotfiles/bin/bin/claude-init
```

---

## Phase 3: Add Interactive Mode

Replace the interactive mode placeholder at the end of `main()` with:
...
### Phase 1
- [ ] `~/dotfiles/bin/bin/templates/` directory exists
- [ ] `CLAUDE.md.template` contains correct content

### Phase 2
- [ ] `claude-init --help` shows usage
- [ ] `claude-init --all` creates full structure
- [ ] `claude-init --minimal` creates only structure + CLAUDE.md
- [ ] `claude-init --update` syncs existing configs
- [ ] `.gitignore` gets updated correctly
- [ ] Error handling works (no source, existing dir)

### Phase 3
- [ ] Interactive prompts work
- [ ] Default answers (Enter) work
...
<project>/
├── CLAUDE.md
├── .gitignore (updated with .claude/settings.local.json, .claude/thoughts/local/)
└── .claude/
    ├── agents/
