...
2. Dispatch a fresh `implementation-specialist` with one issue, its relevant intent and constraints, exact input revisions, required evidence, permitted writes, non-goals, stopping conditions, and report format. Require the specialist to work in an isolated git worktree; never implement or repair product artifacts yourself.
